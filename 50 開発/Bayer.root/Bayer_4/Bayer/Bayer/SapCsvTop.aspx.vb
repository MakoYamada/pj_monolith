...
#Region "SAP用CSV　処理"
    ...
    Private Sub SetKaijoKotsuRecord(ByVal SeikyuData As TableDef.TBL_SEIKYU.DataStruct, _
        ...
        If Kingaku <> 0 Then
            ...
            sb.Append(CmnCsv.SetData(CmnCsv.Quotes(SeikyuData.WBS_ELEMENT)))
            If isTopTour Then
                sb.Append(CmnCsv.SetData(CmnCsv.Quotes(SeikyuData.DANTAI_CODE)))
        ...
        If GetMrRyohiData(SeikyuData.KOUENKAI_NO, SeikyuData.SEIKYU_NO_TOPTOUR, MrData) Then
            ...
            For wCnt As Integer = 0 To UBound(MrData)
                ...
                If Kingaku <> 0 Then
                    ...
                    sb.Append(CmnCsv.SetData(CmnCsv.Quotes(MrData(wCnt).WBS_ELEMENT)))
                    If isTopTour Then
                        sb.Append(CmnCsv.SetData(CmnCsv.Quotes(SeikyuData.DANTAI_CODE)))
                        sb.Append(CmnCsv.SetData(CmnCsv.Quotes(AppModule.GetName_KOUENKAI_DATE(SeikyuData.FROM_DATE, SeikyuData.TO_DATE, True))))
                        sb.Append(CmnCsv.SetData(CmnCsv.Quotes(SeikyuData.KOUENKAI_NO)))
    ...
    Public Function CsvMethod(ByVal intLoop As Integer, ByRef format As String) As String
        ...
        While RsData2.Read()
            ...
            If SeikyuData(wCnt).SRM_HACYU_KBN = AppConst.KAIJO.SRM_HACYU_KBN.Code.No Then
                'SAP精算のとき
                SetKaijoKotsuRecord(SeikyuData(wCnt), csvData, rowCnt, lngTotalKingaku, strSapZeiCd, True, sb)
            End If

            'MR旅費
            SetMrRecord(SeikyuData(wCnt), csvData, rowCnt, lngTotalKingaku, strSapZeiCd, wDB, True, sb)
            'MR宿泊費(2014/12/11 ADD)f
            SetMrHotelRecord(SeikyuData(wCnt), csvData, rowCnt, lngTotalKingaku, strSapZeiCd, wDB, True, sb)
            'タクチケ(課税)
            SetTaxiRecord(SeikyuData(wCnt), csvData, rowCnt, lngTotalKingaku, strSapZeiCd, wDB, True, sb)
