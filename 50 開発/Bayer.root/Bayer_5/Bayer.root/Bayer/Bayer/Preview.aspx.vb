...
Partial Public Class Preview
    ...
    Private Sub PrintSoufujo()
        ...
        '必要に応じマージン設定
        rpt1.PageSettings.Margins.Top = ActiveReport.CmToInch(1.5)
        rpt1.PageSettings.Margins.Bottom = ActiveReport.CmToInch(1.5)
        rpt1.PageSettings.Margins.Left = ActiveReport.CmToInch(1.5)
        rpt1.PageSettings.Margins.Right = ActiveReport.CmToInch(0.9)

        rpt1.KOTSUHOTEL_DATA = RPT_KOTSUHOTEL
        rpt1.LoginID = Session.Item(SessionDef.LoginID)
    ...
    Private Sub PrintDrList()    
        ...
        Else
            DirectCast(rpt1.Sections("PageHeader").Controls("JOKEN_TTANTO"),  _
                    DataDynamics.ActiveReports.TextBox).Text = Joken.TTANTO_ID
        End If
        If Joken.SEND_FLAG <> "指定なし" Then
            DirectCast(rpt1.Sections("PageHeader").Controls("JOKEN_SEND_FLAG"),  _
                    DataDynamics.ActiveReports.TextBox).Text = GetName_SEND_FLAG(Joken.SEND_FLAG, True)
        Else
            DirectCast(rpt1.Sections("PageHeader").Controls("JOKEN_SEND_FLAG"),  _
                    DataDynamics.ActiveReports.TextBox).Text = Joken.SEND_FLAG
        End If
    ...
    Private Sub PrintNewKaijoListReport()
        ...
        '必要に応じマージン設定
        rpt1.PageSettings.Margins.Top = ActiveReport.CmToInch(0.8)
        rpt1.PageSettings.Margins.Bottom = ActiveReport.CmToInch(0.8)
        rpt1.PageSettings.Margins.Left = ActiveReport.CmToInch(0.8)
        rpt1.PageSettings.Margins.Right = ActiveReport.CmToInch(0.8)
    ...
    'NOZOMI送信フラグ
    Private Function GetName_SEND_FLAG(ByVal SEND_FLAG As String, Optional ByVal ShortFormat As Boolean = False) As String
