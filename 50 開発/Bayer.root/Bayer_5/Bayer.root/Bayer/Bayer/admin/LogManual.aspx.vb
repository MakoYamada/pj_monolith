...
Partial Public Class LogManual
    ...
    Private Sub InitControls()
        ...
        AppModule.SetDropDownList_SYORI_NAME.LogManual(Me.JokenSYORI_NAME)
    ...
    Protected Sub GrvList_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GrvList.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            CType(e.Row.Cells(CellIndex.Template1).FindControl("EXPORTIMPORT"), Label).Text = AppModule.GetName_EXPORTIMPORT(e.Row.Cells(CellIndex.INPUT_USER).Text)

            e.Row.Cells(CellIndex.INPUT_DATE).Text = AppModule.GetName_INPUT_DATE(e.Row.Cells(CellIndex.INPUT_DATE).Text)

            CType(e.Row.Cells(CellIndex.Template2).FindControl("STATUS"), Label).Text = AppModule.GetName_STATUS(e.Row.Cells(CellIndex.STATUS).Text)
            Select Case CmnModule.ClearHtmlSpace(e.Row.Cells(CellIndex.STATUS).Text)
                Case AppConst.TBL_LOG.STATUS.Code.OK
                    CType(e.Row.Cells(CellIndex.Template2).FindControl("DivNOTE"), HtmlGenericControl).Visible = False
                Case AppConst.TBL_LOG.STATUS.Code.NG
                    CType(e.Row.Cells(CellIndex.Template2).FindControl("DivNOTE"), HtmlGenericControl).Visible = True
                    CType(e.Row.Cells(CellIndex.Template2).FindControl("NOTE"), Label).Text = e.Row.Cells(CellIndex.NOTE).Text
            End Select
        End If
    End Sub
    ...
    Protected Sub GrvList_RowCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GrvList.RowCreated
        If e.Row.RowType = DataControlRowType.Header OrElse e.Row.RowType = DataControlRowType.Footer OrElse e.Row.RowType = DataControlRowType.DataRow Then
            ...
            e.Row.Cells(CellIndex.INPUT_USER).Visible = False
    ...
    Private Function Check() As Boolean
        ...
        If Not CmnCheck.IsInput(Me.JokenSYORI_NAME) AndAlso _
